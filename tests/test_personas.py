...
from change_gate.personas import PersonaMap, load_persona_map
from change_gate.policy import default_policy_path, load_policy_document
...
def test_policy_file_matches_its_schema():
    ...
    assert default_policy_path() == ROOT / "policy" / "change-gate.policy.json"
...
def test_token_validator_sets_persona_from_azp(signer, pmap):
    ...
    agent = validator.validate(_token(key, azp="change-gate-agent"))
    human = validator.validate(_token(key, azp="change-gate-console",
                                      gate_roles=["change-approver"]))
...
def test_token_validator_without_persona_map_never_grants_a_persona(signer):
    ...
    principal = validator.validate(_token(key, azp="change-gate-agent"))
...
async def test_server_verifier_carries_persona_into_the_tool_principal(signer, pmap,
    ...
    access = await verifier.verify_token(
        _token(key, azp="change-gate-console", gate_roles=["change-approver"])
    )
